...
jQuery.fn.contextPopup = function(menuData, contextmenutypepxST) {
    ...
    function createMenu(e, tinythis) {
        ...
        settingspretty.items.forEach(function(item) {
            if (item) {
                if ((item.href == null) || (item.href.startsWith('(function ')) || (item.href.startsWith('javascript:'))) { var item_final_href = ''; } else { var item_final_href = 'href="' + item.href + '" '; }
                if (item.target == null) { var item_final_target = ''; } else { var item_final_target = ' target="' + item.target + '" '; }
                ...
                if (item.isEnabled != undefined && !item.isEnabled()) {
                    ...
                    row.find('.' + settingspretty.linkClickerClass)
                        .data("contextMenuPlugin-main", tinythis)
                        .data("contextMenuPlugin-data", item.data)
                        .click(function() { item.action(e, this); });
                }
    ...
    if (contextmenutypepxST == null) { var contextmenutypepx = "contextmenu"; } else { var contextmenutypepx = contextmenutypepxST; }

    // On contextmenu event (right click)
    this.on(contextmenutypepx, function(e) {

        var menupretty = createMenu(e, this)
            .show();
